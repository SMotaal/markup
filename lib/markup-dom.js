...
export function* renderer(tokens, tokenRenderers = renderers) {
    ...
    // if (NESTED_MODE) {
      // const root = Element('slot', {className: 'markup'});
      // let parent,
      //   parentElement = root;
      // for (const token of tokens) {
      //   const {type = 'text', text, punctuator} = token;
      ...
      //     // parentElement === root && (yield element);
      //   }
      //   // element && (yield element);
      // }
      // yield root;
...
//   let parent, root;
//   const root = parentElement = Element('slot', {className: 'markup'});
...
const Text =
  Element === createElement
    ...
    : (content = '') => String(content).replace(/[\u00A0-\u9999<>\&]/gim, v => `&#${v.charCodeAt(0)};`);
