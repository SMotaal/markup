...
Object.assign(syntaxes, {
  ...
  es: {
    syntax: 'es',
    keywords: symbols(
      'enum interface module namespace type class function const let var break continue debugger do export for import return switch while with declare abstract async as case catch default if else extends finally from in of await delete instanceof new typeof void this get set',
    ),
    assigners: symbols('= += -= *= /= **= %= |= ^= &= <<= >>= >>>='),
    operators: symbols('++ -- !! + - ^ ~ ! ...'),
    combinators: symbols('>= <= == === != !== || && ! & | > < => % ** * / >> << >>> ? : .'),
