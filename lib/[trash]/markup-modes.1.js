...
Syntaxes: {
  ...
  Markdown: {
    ...
    const md = (syntaxes.md = {
      ...
      comments: closures('<!--…-->'),
      quotes: [],
      closures: closures(`${html.closures} ${'```…\n```'}`), // '**…** *…* ~~…~~ _…_ `…` ```…``` ~~~…~~~ #…\n ##…\n ###…\n ####…\n #####…\n'
    ...
    md.closures['<'].closer = html.closures['<'].closer;
    md.closures['<'].close = html.closures['<'].close;

    FencdCodeBlocks: {
      const FENCE = '(?:```|~~~)';
      const LEAD = raw`\s*`;
      const TAIL = raw`(?:\s+.*)?\s*`;
      const SYNTAX = raw`[a-zA-Z]\w*|`;
      const CLOSER = new RegExp(raw`^(${LEAD})(${FENCE})(${TAIL})$`, 'mg');
      const OPENER = new RegExp(raw`^(${LEAD})(${FENCE})(${SYNTAX})(${TAIL})$`, 'mg');

      // md.closures['```'].quotes = symbols(`' "`);
      md.closures['```'].closer = CLOSER;
      md.closures['```'].close = (next, state, grouper) => {
        const parent = next.parent;
        const first = parent.next;
        const fence = first.text;
        const lead =
          (parent.previous &&
            parent.previous.type === 'whitespace' &&
            raw`${parent.previous.text}`) ||
          '';
        let {source, index} = state;
        const $$matcher = syntaxes.md.patterns.codeFence;
        // const $$closer = new RegExp(raw`(${lead})(${fence})(${TAIL})`, 'mg');
        let match = $$matcher.exec(source);
        $$matcher.lastIndex = index;
        if ((match = $$matcher.exec(source))) {
          ...
          const text = source.slice(offset, match.index - 1);
          state.index = match.index;
          next = [{text, offset, previous: next, parent: first.parent}];
        }
        console.log('Close %o — %o', fence, next);
        return next;
        // while ((match = $$matcher.exec(source))) {
        //   if ($$closer.test(match[0])) {
        //     const offset = index;
...
//   const hasHead = parent.next && !parent.next.breaks;
//   const hasSyntax = hasHead && SYNTAX.text(parent.next.text) &&
//     parent.next.next && !(parent.next.next.text && parent.next.next.text.trim());

//   if (hasSyntax) {
//     const tokens = [token.text];
//     while ((token = token.next) && !token.breaks) token.text && token.text.trim() && tokens.push(token.text);
//     const [head, syntax, attributes] = HEAD.exec(tokens) || '';
//   }
