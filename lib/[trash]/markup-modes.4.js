...
export const install = (defaults, newSyntaxes = defaults.syntaxes || {}) => {
  ...
  // Object.defineProperties(newSyntaxes, Object.getOwnPropertyDescriptors(syntaxes));
...
Syntaxes: {
  ...
  HTML: {
...

    {
      ...
      const closure = html.closures.get('<');

      closure.close = (next, state, context) => {
  ...
  Markdown: {
    ...
    if (md.closures) {
      ...
      const EMBEDDED = true;
      {
        ...
        for (const opener of ['```', '~~~']) {
          const closure = md.closures.get(opener);
          if (closure) {
            closure.matcher = new RegExp(
  ...
  ECMAScript: {
    ...
    const es = (syntaxes.es = {
      ...
      matcher: sequence`([\s\n]+)|(${all(
        ...
        /,|;|\.\.\.|\.|\:|\?|=>/,
        /!==|===|==|=/,
        ...Symbols.from(raw`\+ \- \* & \|`).map(s => `${s}${s}|${s}=|${s}`),
        ...Symbols.from(raw`! \*\* % << >> >>> < > \^ ~`).map(s => `${s}=|${s}`),
      )})`,
      matchers: {
        quote: /(\n)|(\\(?:(?:\\\\)*\\|[^\\\s])?|`|"|'|\$\{)/g,
        // quote: /(\n)|(`|"|'|\$\{)|(\\.)/g,
...
// const LINE = /$/g;
// closures: string => {
//   const pairs = helpers.symbols(string);
//   const array = new Array(pairs.length);
//   array.pairs = pairs;
//   let i = 0;
//   for (const pair of pairs) {
