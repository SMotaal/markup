...
export const ready = (async () => {
  // await patterns.ready;
  await modes.ready;
  // return;
  // const maybeIdentifier = modes.syntaxes.es.patterns.maybeIdentifier = patterns.identifier(
  //   patterns.expressions.es.IdentifierStart,
  //   patterns.expressions.es.IdentifierPart,
  // );
...
const initialize = () =>
  ...
  (initialized = async () => {
    ...
    const {createFragment, supported, document} = dom;
...
export const warmup = async (source, options) => {
  ...
  cache.has(source) || ([...tokenize(source, options)], cache.add(source));
  return true;
};
