...
const premap = context => {
  if (context && !premap.contexts.has(context)) {
    const {assign} = Object;
    const updates = {};
    const groups = {};
    const maps = premap.maps;
    for (const [map, mappings] of maps) {
      for (const [type, group] of mappings) {
        const symbols = (groups[group] = context[group]);
        let mapped;
        if (symbols && symbols.size) {
          for (const symbol of symbols) {
            if (symbol && !(mapped && symbol in mapped)) {
              (mapped || (mapped = {}))[symbol] = type;
            }
          }
        }
        if (mapped) {
          updates[map] ? assign(updates[map], mapped) : (updates[map] = mapped);
        } else {
    ...
    console.log({groups, updates, maps});
    premap.contexts.add(assign(context, updates));
  }
  return context;
};

{
  const pluralize = s => ((s = s.split(':')), ([, s[1] = `${s[0]}s`] = s));
  const map = m => m.split(' ').map(pluralize);
  premap.maps = Object.entries({
    aggregators: map('nonbreaker operator comment quote closure breaker'),
    punctuators: map('assigner combinator'),
  });
  premap.contexts = new WeakSet();
}
...
export function* contextualizer($, defaults) {
  ...
  const {
    ...
    aggregators: $aggregators, // punctuators: {aggregators: $aggregators},
  } = $;
...
export function* tokenizer(context) {
  ...
  while (!done) {
    ...
    if (next && next.text) {
      ...
      if (type === 'sequence') {
        // (aggregate && previous && (aggregators[text] || (!(text in aggregators) && (aggregators[text] = aggregate(text))))) ||
        // (punctuators[text] ||
        //   (!(text in punctuators) && (punctuators[text] = punctuate(text)))) ||
        // undefined) && (next.type = 'punctuator');
        (next.punctuator =
          (previous && aggregators && aggregators[text]) ||
          (punctuators && punctuators[text]) ||
          (spans && spans.includes(text) && 'span') ||
          undefined) && (next.type = 'punctuator');
...
// // if (!('aggregators' in punctuators)) {
// if (!aggregators) {
//   const {assigners, combinators} = $;
//   for (const [type, symbols] of [['assigner', assigners], ['combinator', combinators]]) {
//     if (symbols && symbols.size > 0) {
//       for (const symbol of symbols) {
//         !aggregators ||
//           symbol in aggregators ||
//           ((aggregators || (aggregators = {}))[symbol] = type);
//       }
//     } else {
//       $[type] = undefined;
//     }
//   }
//   aggregators = punctuators.aggregators = aggregators || undefined;
//   if (aggregators) console.log(aggregators);
// }
...
// const map = mappings => {
//   let map;
//   for (const [key, symbols] of Object.entries(mappings)) {
//     if (symbols && symbols.size > 0) {
//       const type = key.replace(/ies$|s$/, '');
//       for (const symbol of symbols) {
//         (map ? !(symbol in map) : (map = {})) && (map[symbol] = type);
//       }
//     } else {
