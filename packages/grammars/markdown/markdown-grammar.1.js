...
export const markdown = (() => {
  ...
  const markdown = ({syntax} = defaults, {html}) => {
    const EMBEDDED = true;
    ...
    const mode = {
      ...
      matcher: matcher,
      // spans: Closures.from('``…`` `…`'),
      spans: Closures.from(markdown.SPANS),
      matchers: {comment: /(\n)|(-->)/g},
    };
  ...
  Object.defineProperty(markdown, 'default', {get: () => ({...defaults})});
