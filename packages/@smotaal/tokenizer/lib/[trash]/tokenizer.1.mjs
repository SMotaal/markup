...
export class Tokenizer {
  ...
  *tokenize(source, state = {}) {
    ...
    while (true) {
      ...
      const hint = grouping.hint || syntax;
      ...
      while (lastContext === (lastContext = context)) {
        ...
        if (punctuator || closing) {
          ...
          if (closing) {
            // ({closed, after, parent} = grouping.close(next, parent, state, context));
            ({after, closed, grouper} = grouping.close(next, state, context));
            ...
            ({grouper, opened, parent = parent, punctuator} = grouping.open(next, context));

            // !grouper || next.punctuator === grouper.punctuator || (next.punctuator = grouper.punctuator);
