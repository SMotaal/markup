...
// (/^[a-z]+:/.test(referrer) && referrer) || `file:///${referrer || ''}`
// async function bundle(...modules) {
//   const input = modules[0];
//   const bundle = await rollup.rollup({...options, input, plugins});
//   const {code: source, map: map} = await bundle.generate(options);
//   bundle.output = {url: `${new URL(input, base)}`, source, map, type};
//   return bundle;
// }

// async function read(module) {
//   const cache = read.cache || (read.cache = []);
//   const request = cache[module] || (cache[module] = fetch(module));
//   try {
//     sources[module] = await (sources[module] = request.then(response => response.text()));
//   } catch (exception) {
//     (sources[module] = ''), error(exception);
//   }
//   return request;
// }

// async function write(source) {
//   if (!cache || typeof cache !== 'object') throw Error('Cannot write - no cache');
//   const {url, blob = (source.blob = await serialize(source))} = source;
//   (await cache).put(url, new Response(blob));
...
//   const {
//     blob = (source.blob = await serialize(source)),
//     blob: {url = (blob.url = URL.createObjectURL(blob))},
//   } = source;
//   if (typeof importScripts === 'function') importScripts(url);
//   else if (typeof Worker === 'function') new Worker(url);
