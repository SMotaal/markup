  ...
  while (!done) {
    ...
    if (next) {
      ...
      if (sequence) {
        ...
        // (keywording && $keywords.includes(word) && (type = 'keyword')) ||
        //   (maybeIdentifier && maybeIdentifier.test(word) && (type = 'identifier'));
        // // || (maybeKeyword && maybeKeyword.test(word) && (type = 'word'));
      ...
      token = {type, text, offset, breaks, hint, previous, parent};
    }
...
export function* tokenize(source, state = {}, defaults = markup.defaults) {
  ...
  while (!done) {
    ...
    while (lastContext === (lastContext = $context)) {
      ...
      let body;
      let punctuator = sequence && type !== 'seqeunce' && next.type;
