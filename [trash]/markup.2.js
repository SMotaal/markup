...
export function* tokenize(source, state = {}) {
  ...
  while (!done) {
    ...
    while ($syntax === $.syntax) {
      ...
      const $$grouper = state.grouper; // closer && groupings[groupings.length - 1];
      const $$closer = $$grouper && $$grouper.closer; // closers.length && closers[closers.length - 1];
      ...
      if (punctuator || closing) {
        ...
        if (closing) {
          closed = grouper = closing && groupings.pop(); // closers.pop(),
          grouper &&
          ...
          if (opened) {
            ...
            hints.add(hinter); // hinters.push(hinter), // closers.push(grouper.closer)
        ...
        if (opened || closed) {
          // state.closer = (state.grouper = grouper) ? grouper.closer : undefined;
          state.grouper = grouper;
          state.hint = hint = [...hints].join(' ');
        }
