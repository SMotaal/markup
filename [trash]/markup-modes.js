...
Object.assign(syntaxes, {
  ...
  es: {
    syntax: 'es',
    keywords: symbols(
      // abstract enum interface type module namespace declare
      'class function arguments const let var break continue debugger do export for import return switch while with yield async as case try catch throw finally default if else extends from in of await delete instanceof new typeof void this get set',
    ),
    assigners: symbols('= += -= *= /= **= %= |= ^= &= <<= >>= >>>='),
    combinators: symbols('>= <= == === != !== || && ! & | > < => % ** * / >> << >>> ? :'),
    nonbreakers: symbols('.'),
...
// {
//   syntaxes.es.matchers.spans = syntaxes.es.matcher;
//   console.log(syntaxes.es.matcher);
// }
...
/** @see https://github.com/acornjs/acorn/blob/2adf569de6f77e2f9971d944669ec70071acfb30/acorn/src/identifier.js#L30-L36 */
{
  ...
  console.log('syntaxes.es.patterns: %o', syntaxes.es.patterns);
  console.log('syntaxes.css.patterns: %o', syntaxes.css.patterns);
}
