import {patterns, matchers, modes} from './markup-parser.js';
...
export const install = (defaults, newSyntaxes = defaults.syntaxes || {}) => {
  // newModes = defaults.modes || {}
  Object.assign(newSyntaxes, syntaxes);
  defaults.syntaxes === newSyntaxes || (defaults.syntaxes = newSyntaxes);
  // Object.assign(newModes, modes);
  // defaults.modes === newModes || (defaults.modes = newModes);
};
...
const Sequence = (pattern, flags = 'g') => new RegExp(pattern, flags);
...
//   patterns.map(pattern => (pattern && pattern.source) || pattern).join('|'),
//   'g',
// );
...
const QUOTES = /`|"|'/g;
const CLOSURES = /\{|\}|\(|\)|\[|\]/g;
const ENCLOSURES = Sequence(
  [QUOTES.source, CLOSURES.source, REGEXPS.source, COMMENTS.source].join('|'),
);
const TOPLEVEL = Sequence(raw`([\s\n]+)|(${ENCLOSURES.source})`);
...
/** @see https://github.com/acornjs/acorn/blob/2adf569de6f77e2f9971d944669ec70071acfb30/acorn/src/identifier.js#L30-L36 */
{
  ...
  const maybeECMAScriptIdentifier = Identifier(
  ...
  syntaxes.es.patterns.maybeIdentifier = maybeECMAScriptIdentifier;
}

/// ESx
{
  const {comments, quotes, closures, spans, matcher, matchers} = syntaxes.es;
  // const REGEXPS = /\/(?=[^\*\/\n][^\n]*\/)(?:[^\\\/\n\t\[]+|\\\S|\[(?:\\\S|[^\\\n\t\]]+)+?\])+?\//g; // |\/=|\/\/|\/\*|\*\/|\/
  // const REGEXPS = /\/(?=.*\/)(?:[^\\\/\n\t\[]+|\\\S|\[(?:\\\S|[^\\\n\t\]]+)+?\])+?\//g; // |\/=|\/\/|\/\*|\*\/|\/
  // const COMMENTS = /\/\/|\/\*|\*\/|\//g;
  // const QUOTES = /`|"|'/g;
  ...
  const base = {
    matchers: {comment: matchers.comment, quote: matchers.quote},
    syntax: {comments, quotes, closures, spans, matcher: TOPLEVEL},
  };
